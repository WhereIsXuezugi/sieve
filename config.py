...
@dataclass
class Config:
    data_dir: Path = Path(os.environ.get("SIEVE_DATA", "./data"))
    host: str = "127.0.0.1"
    port: int = 8377
    ...
    def load(cls, path: str | os.PathLike | None = None) -> Config:
        cfg = cls()
        candidates = [path] if path else ["sieve.toml", "config.toml", "/etc/sieve/config.toml"]
...
def resolve_settings(stored: dict | None) -> dict:
    ...
    merged = deep_merge(DEFAULT_SETTINGS, stored or {})
